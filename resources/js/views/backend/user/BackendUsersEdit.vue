        ...
        <div class="bg-white p-16 rounded shadow-2xl w-2/3">
            ...
            <form @submit.prevent="editUser">
                ...
                <div>
                    ...
                    <p class="w-full border-2 border-gray-200 p-3 rounded outline-none focus:border-purple-500">{{user.email}}</p>
                </div>
                <div>
                    <label class="block mb-1 font-bold text-gray-500">Username</label>
                    <p class="w-full border-2 border-gray-200 p-3 rounded outline-none focus:border-purple-500">{{user.username}}</p>
                </div>
                <div>
...
export default {
    ...
    created() {
        let url = "/api/admin/users/" + this.$route.params.id;
        axios
            .get(url)
            .then((result) => {
                console.log(result);
    ...
    methods: {
        editUser() {
            let url = "/api/admin/users/update/" + this.$route.params.id;
            let json = {
                name: this.name,
