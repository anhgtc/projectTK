<template>
    <div>
                    ...
                    <div class="bg-white shadow-md rounded my-6">
                        <table class="min-w-max w-full table-auto text-center ">
                            ...
                            <tbody class="text-gray-600 text-sm font-light">
                                <tr v-for="(post, index) in posts" :key="index" class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex item-center justify-center">
                                            <router-link :to="{name:'BackendPostsEdit', params:{id: post.id}}">
        ...
        <router-link class="fixed bottom-10 right-10" :to="{name:'BackendPostsViewCreate'}">
...
export default {
    ...
    created() {
        ...
        axios
            .get(url)
            .then((result) => {
                if (result.status == 200) this.posts = result.data.posts;
                else alert('That bai')
            })
            .catch((error) => {
    ...
    methods: {
        deletePost($id, $index) {
            let url = "/api/admin/posts/delete/" + $id;
            axios
                .post(url)
                .then((result) => {
